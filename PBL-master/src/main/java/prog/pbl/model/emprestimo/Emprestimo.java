...
public class Emprestimo implements Serializable {
    ...
    public Emprestimo(Leitor leitor, Livro livro) throws EmprestimoException {
        try{
            Emprestimo emprestimo1 = MasterDao.getEmprestimoDao().findByUserAndLivro(livro.getIsbn(), leitor.getId());
            if (!emprestimo1.isDevolvido()){
                throw new EmprestimoException("Este Emprestimo ja existe e esta ativo", emprestimo1);
            } else {
                throw new EmprestimoException("Emprestimo inativo", null);
            }
        } catch (EmprestimoException e) {
            if (Objects.equals(e.getMessage(), findEmprestimo) || Objects.equals(e.getMessage(), "Emprestimo inativo" )|| Objects.equals(e.getMessage(), findWhenNotHaveObj)) {

                try {
                    if (leitor.getNumEmprestimos() <= 0) {
                    ...
                    } else if (leitor.getDiasRestantesMulta() > 0) {
                        throw new EmprestimoException(createEmprestimoWithMulta, null);
                    } else if (livro.getQuantidade() <= 0) {
                        throw new EmprestimoException(createEmprestimoWithNoHaveBookAvaliable, null);
